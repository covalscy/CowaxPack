package com.cowax.cowaxpack.client.screens;
...
public class ArtilleryIndicatorScreen extends Screen {
    ...
    private static final ResourceLocation TEXTURE = new ResourceLocation(CowaxPack.MODID, "textures/gui/artillery_indicator.png");
    ...
    public ArtilleryIndicatorScreen(ItemStack stack, InteractionHand hand) {
        super(Component.translatable("item.cowaxpack.artillery_indicator"));
        this.stack = stack;
        this.hand = hand;
        if (!stack.isEmpty()) {
    ...
    protected void renderBg(GuiGraphics pGuiGraphics, int pMouseX, int pMouseY) {
        ...
        if (pMouseX >= i + 98 && pMouseX <= i + 162 && pMouseY >= j + 19 && pMouseY <= j + 49) {
            pGuiGraphics.renderTooltip(this.font,
                    this.isDepressed ?
                            Component.translatable("tips.cowaxpack.mortar.target_pos.depressed_trajectory").withStyle(ChatFormatting.WHITE) :
                            Component.translatable("tips.cowaxpack.mortar.target_pos.lofted_trajectory").withStyle(ChatFormatting.WHITE),
                    pMouseX, pMouseY);
        }
    ...
    class DoneButton extends AbstractButton {
        ...
        public void onPress() {
            ...
            CowaxPack.NETWORK.sendToServer(
