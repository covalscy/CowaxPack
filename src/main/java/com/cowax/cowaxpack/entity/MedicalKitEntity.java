...
import static com.cowax.cowaxpack.item.common.MedicalKitItem.treat;

public class MedicalKitEntity extends Entity implements GeoEntity {

    private static final int DESPAWN_TICKS = 15 * 20;
    ...
    public void tick() {
        ...
        if (!this.level().isClientSide() && this.tickCount >= DESPAWN_TICKS) {
            this.discard();
            return;
        }
        ...
        // Проверку подбора делаем только на сервере и не каждый тик (раз в 4 тика)
        if (!this.level().isClientSide() && this.tickCount >= 10 && (this.tickCount & 3) == 0) {
            touchEntity();
        }
    }
    ...
    public void touchEntity() {
        if (level() instanceof ServerLevel serverLevel) {
            ...
            List<LivingEntity> entities = level().getEntities(EntityTypeTest.forClass(LivingEntity.class), frontBox, living -> living.getHealth() < living.getMaxHealth());
